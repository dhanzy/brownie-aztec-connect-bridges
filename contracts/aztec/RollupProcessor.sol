...
contract RollupProcessor is DSTest {
    ...
    function setBridgeGasLimit(address bridgeAddress, uint256 gasLimit)
        external
    {
        bridgeGasLimits[bridgeAddress] = gasLimit;
    }

    function getBridgeGasLimit(address bridgeAddress)
        private
        view
    ...
    function getDefiResult(uint256 nonce)
        public
        returns (bool finalised, uint256 outputValueA)
    {
    ...
    function _convert(ConvertArgs memory convertArgs)
        private
        returns (ConvertReturnValues memory results)
    {
        ...
        if (success) {
            (uint256 outputValueA, uint256 outputValueB, bool isAsync) = abi
                .decode(result, (uint256, uint256, bool));
            if (!isAsync) {
                emit DefiBridgeProcessed(
                    0,
                    convertArgs.interactionNonce,
    ...
    function getDefiInteractionBlockNumber(uint256 interactionNonce)
        external
        view
