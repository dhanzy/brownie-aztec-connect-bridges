...
// 2. You can find the readme for the contract here: https://g ist.github.com/realdiganta/2c73f86820bf7310bd934184fa960e3d
...
contract RaiBridge is ERC20, IDefiBridge {
    ...
    address public constant SAFE_ENGINE =
        0xCC88a9d330da1133Df3A7bD823B95e52511A6962;
    address public constant SAFE_MANAGER =
        0xEfe0B4cA532769a3AE758fD82E1426a03A94F185;
    address public constant COIN_JOIN =
    ...
    constructor(
        ...
        require(
            IWETH(WETH).approve(ETH_JOIN, type(uint256).max),
            "Weth approve failed"
        );
        require(
            IWETH(WETH).approve(rollupProcessor, type(uint256).max),
            "Weth approve failed"
        );
        require(
            IERC20(RAI).approve(COIN_JOIN, type(uint256).max),
            "Rai approve failed"
        );
        require(
            IERC20(RAI).approve(rollupProcessor, type(uint256).max),
            "Rai approve failed"
        );
        ...
        )
    {
        ...
        if (safe.lockedCollateral != 0 || safe.generatedDebt != 0) {
            collateralRatio = safe.lockedCollateral.mul(1e22).div(raiToEth).div(
                    safe.generatedDebt
                );
        } else {
            collateralRatio = 0;
    ...
    function _addCollateral(
        ...
        ISafeManager(SAFE_MANAGER).modifySAFECollateralization(
            safeId,
            int256(_wethAmount),
            int256(outputRai)
        );

        ISafeManager(SAFE_MANAGER).transferInternalCoins(
            safeId,
            address(this),
            outputRai * 10**27
        );
    ...
    function _removeCollateral(uint256 _raiAmount, ISafeEngine.SAFE memory safe)
        internal
        returns (uint256 outputWeth)
    {
        ICoinJoin(COIN_JOIN).join(SAFE_HANDLER, _raiAmount);

        outputWeth = safe.lockedCollateral.mul(_raiAmount).div(
            safe.generatedDebt
        );
        ...
        ISafeManager(SAFE_MANAGER).transferCollateral(
            safeId,
            address(this),
            outputWeth
        );
